...
- **Status:** ✅ **shipped** — bidirectional `EvalLog` interop layer (`ascore.interop`), `GET /api/scorecards/{id}/inspect.json` + `ascore inspect-export`/`inspect-import`. Lossless on the mappable fields; validated against real `inspect_ai`. See [INSPECT_INTEROP.md](INSPECT_INTEROP.md).
