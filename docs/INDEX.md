...
| [ADDING_AN_AGENT.md](ADDING_AN_AGENT.md) | The integration ladder: HTTP mapping -> ACP -> a declarative CLI spec -> OTel correlation, and only then a module. Adding a subject is a `config.yaml` entry, not a source change | You want to test an agent that isn't wired up yet |
...
| [RELEASE.md](RELEASE.md) | How a release actually reaches node1: the four rules (never build on the VM, never run deploy.sh against it, never touch config.yaml/.env, always pass both profiles), pre-flight checks, the build-locally/ship-the-image steps, verification incl. the signing kids, rollback, failure signatures, and how to recover the host-only compose override | You are shipping to production |
