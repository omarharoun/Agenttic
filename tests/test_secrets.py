...
def test_get_secret_prefers_file(tmp_path, monkeypatch):
    ...
    assert get_secret("AGENTTIC_API_TOKEN") == "file-secret"


def test_get_secret_env_when_no_file(monkeypatch):
    monkeypatch.delenv("AGENTTIC_API_TOKEN_FILE", raising=False)
    monkeypatch.setenv("AGENTTIC_API_TOKEN", "env-secret")
    assert get_secret("AGENTTIC_API_TOKEN") == "env-secret"
...
class TestRedaction:
    def test_redacts_message_and_extra_fields(self):
        ...
        rec = logging.LogRecord("agenttic", logging.INFO, __file__, 1,
